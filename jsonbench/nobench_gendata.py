...
def main_non_cli(num_objs=0, mongo=False, outfile="generic_nobench_data", extra_file=""):
    if num_objs < 1:
        print
    ...
    if len(extra_file) > 0:
        secondary_outfile_name = extra_file
    else:
        outfile_name_parts = outfile.rsplit(".", 1)
        if len(outfile_name_parts) == 2:
            secondary_outfile_name = (oufile_name_parts[0] + "_extra" + "." + outfile_name_parts[1])
        else:
            secondary_outfile_name = outfile_name_parts[0] + "_extra"
